...
describe('Security Testing - Authentication Bypass', () => {
  ...
  describe('Input Sanitization', () => {
    it('should prevent XSS in API responses', async () => {
      ...
      const request = new NextRequest(`http://localhost:3000/api/protected?message=${encodeURIComponent(maliciousInput)}`)
      const response = await handler(request)
    ...
    it('should prevent SQL injection in parameters', async () => {
      ...
      const request = new NextRequest(`http://localhost:3000/api/users?userId=${encodeURIComponent(sqlInjection)}`)
      const response = await handler(request)
