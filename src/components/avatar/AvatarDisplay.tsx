...
function SimpleAvatar({ viseme, state, className }: { viseme: Viseme; state: AvatarState; className?: string }) {
  ...
  return (
    <div className={cn('relative flex items-center justify-center', className)}>
      ...
      >
        ...
        >
          {/* Face circle */}
          <circle cx="50" cy="50" r="40" fill="hsl(var(--primary) / 0.1)" />
        ...
        <div className="absolute -bottom-2 left-1/2 -translate-x-1/2">
          <div
            className={cn(
              ...
              state === 'listening' && 'bg-blue-500/20 border-blue-500 text-blue-700 dark:text-blue-300',
              state === 'thinking' && 'bg-yellow-500/20 border-yellow-500 text-yellow-700 dark:text-yellow-300 animate-pulse',
              state === 'speaking' && 'bg-green-500/20 border-green-500 text-green-700 dark:text-green-300'
            )}
          >
            {state === 'idle' && '💤 Idle'}
      ...
      {state === 'listening' && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="w-56 h-56 rounded-full border-2 border-primary/30 animate-ping" />
        </div>
      )}
    </div>
  );
