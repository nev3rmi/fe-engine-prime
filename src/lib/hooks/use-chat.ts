...
import type { UseChatReturn, ChatMessage, NewChatMessage, OnlineUser } from "@/types/realtime";
...
export const useMessageReactions = (messageId: string) => {
  const [reactions, setReactions] = useState<
    Array<{ emoji: string; users: string[]; count: number }>
  >([]);
  ...
  useEffect(() => {
    ...
    socket.on(
      "message:reaction",
      ({ messageId: reactionMessageId, reactions: updatedReactions }) => {
