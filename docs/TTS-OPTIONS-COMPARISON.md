...
This document compares all available Text-to-Speech (TTS) options for the
...
Uses the built-in browser speech synthesis (Web Speech API). Works client-side
...
Microsoft's cloud-based TTS service with neural voices and native viseme data
support.
...
- ✅ **Vietnamese Support** - 2 neural voices: `vi-VN-HoaiMyNeural` (F),
  `vi-VN-NamMinhNeural` (M)
...
**To Fix**: Add Azure API key → Instant Vietnamese TTS!
...
**The model you shared (`vietnamese-sbert`) is not a TTS model** - it's for text
similarity/embeddings, not speech generation.
